...
<table width="100%" height="123" border="0" cellpadding="0" cellspacing="2">
    <tr>
        ...
        <td colspan="4" align="left">
            <input type="button" name="load_data" id="load_data" class="btn btn-sm btn-info" value="Load Data" tabindex="10" onclick="show_list_view2(JSON.stringify({
                'user_id':document.getElementById('cbo_user_name').value,
                'module_id':document.getElementById('cbo_main_module').value,
                'blade': 'load_priv_list_view'

            }),'tools/load_priv_list_view','load_list_priv',JSON.stringify({'blade': 'load_priv_list_view'}))" />
        </td>
