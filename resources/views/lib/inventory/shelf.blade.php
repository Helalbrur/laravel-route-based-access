...
<div class="row mb-2">
    <div class="col-sm-12 d-flex justify-content-center">
        <h1 class="m-0 text-center"><strong>{{getMenuName(request('mid') ?? 0) ?? 'Shelf'}}</strong></h1>
    </div>
</div>
@endsection()
@section('content')
                ...
                <div class="card-body">
                    <div class="card-text">
                        <div class="card pt-4 px-4" style="background-color: rgb(241, 241, 241);">
                            <form name="libShelf_1" id="libShelf_1" autocomplete="off">
                                <div class="row">
                                    <div class="col-sm-12">
                                        <div class="form-group row d-flex justify-content-center">
                                            <label for="txt_shelf_no" class="col-sm-3 col-form-label fw-bold text-start must_entry_caption">Shelf No</label>
                                            <div class="col-sm-8">
                                                <input type="text" class="form-control" id="txt_shelf_no" name="txt_shelf_no">
                                            </div>
                                        </div>
                                        <div class="form-group row d-flex justify-content-center">
                                ...
                                <div class="row d-flex justify-content-center mt-4">
                                    ...
                                    <div class="col-sm-8">
                                        <?php echo load_submit_buttons($permission, "fnc_lib_shelf", 0, 0, "reset_form('libShelf_1','','',1,'')"); ?>
                                    </div>
                                </div>
                            </form>
                        </div>
                                ...
                                <tbody id="list_view">
                                    ...
                                    $shelfs = DB::table('lib_floor_room_rack_mst as a')
                                        ...
                                        ->select(
                                            ...
                                            'a.floor_room_rack_name as shelf_no',
...
<script>
    ...
    function fnc_lib_shelf( operation )
    {
        if (form_validation('txt_shelf_no*cbo_company_name*cbo_location_name*cbo_store_name*cbo_floor_name*cbo_room_no*cbo_rack_no','Shelf No* Company Name*Location*Store Name*Floor Name*Room No*Rack No')==false)
        {
            return;
        }
        else
        {
            var formData = get_form_data('txt_shelf_no,cbo_company_name,cbo_location_name,cbo_store_name,cbo_floor_name,cbo_room_no,update_id,cbo_rack_no');
            var method ="POST";
            var param = "";
            ...
            var url = `/lib/inventory/shelf${param}`;
            var requestData = {
                method: method,
                headers: {
            ...
            save_update_delete(operation,url,requestData,'id','show_shelf_list_view','list_view_div','libShelf_1');
        }
    }
    ...
    async function load_php_data_to_form(rack_id) {
        try {
            freeze_window(3);
            reset_form('libShelf_1', '', '', 1);
            const response = await fetch(`/shelf_details/${rack_id}`);
            ...
            $('#cbo_rack_no').val(data.rack_id).trigger('change');

            document.getElementById('txt_shelf_no').value = data.shelf_no;
            document.getElementById('update_id').value = data.id;

            set_button_status(1, permission, 'fnc_lib_shelf', 1);
            release_freezing();
        } catch (error) {
    ...
    async function handleRoomChange() {
        try {
            await load_drop_down_v2('load_drop_down', JSON.stringify({'room_id':document.getElementById('cbo_room_no').value,'onchange':''}), 'rack_under_room', 'rack_div');
        } catch (error) {
            console.error('Error loading dropdown:', error);
        }
    }
